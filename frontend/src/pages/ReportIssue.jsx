...
export default function ReportIssue() {
  ...
  const fetchAddress = async (latVal, lngVal) => {
    try {
      ...
      if (res.ok) {
        ...
        let roadName = address.road || address.pedestrian || address.suburb || address.neighbourhood || address.highway || address.amenity || address.village || address.town || "";
        if (!roadName) {
          if (address.city || address.county || address.state) {
            roadName = `Location near ${address.city || address.county || address.state}`;
          } else {
            roadName = `Location at ${latVal.toFixed(4)}, ${lngVal.toFixed(4)}`;
          }
        }
  ...
  const handleLocationChange = (newLat, newLng) => {
    setLat(newLat);
    setLng(newLng);
    setRoad(`Location at ${newLat.toFixed(4)}, ${newLng.toFixed(4)}`);
